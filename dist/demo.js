(function () {
  ...
  const chooseName = () => {
  	var names = [];
  	for ( var i in window             ) if( i.length < 6 ) names.push(i);
  	for ( var i in Object.prototype   ) if( i.length < 6 ) names.push(i);
  	for ( var i in Function.prototype ) if( i.length < 6 ) names.push(i);
  	for ( var i in Array.prototype    ) if( i.length < 6 ) names.push(i);
  	for ( var i in String.prototype   ) if( i.length < 6 ) names.push(i);
  ...
  var tokens; 
  if( confirm('create?') ){ 
  	var paircode = chooseName();
  	tokens = getTokens(paircode);
  	alert("paircode = "+paircode);
  }else {
  	var paircode = prompt('enter paircode:');	
  	tokens = getTokens(paircode);
  }
  const provider = new WebrtcProvider( tokens.room, ydoc,{ password: tokens.pass, maxConns:1 });
  const keys$1 = ydoc.getArray('array');
  ...
  keys$1.observe(event => {
  	//console.log(event.changes.delta);
  	var q = keys$1.toArray();
  	q.map( (key) => {
  		if(key == 65 ) o.style.left = (parseInt(o.style.left.replace('px','')) - 5)+'px';
  		if(key == 68 ) o.style.left = (parseInt(o.style.left.replace('px','')) + 5)+'px';
  		if(key == 87 ) o.style.top  = (parseInt(o.style.top.replace('px','')) - 5)+'px';
  		if(key == 83 ) o.style.top  = (parseInt(o.style.top.replace('px','')) + 5)+'px';
  	});
  	keys$1.delete(0, q.length-1 );
  });

  window.addEventListener('keydown',  function(ev){
  	keys$1.insert(keys$1.length, [ev.which]);
  });
